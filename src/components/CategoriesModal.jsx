...
const CategoriesModal = ({ language, getLandmarks, categories, setIsCategoriesModalOpen, selectedCategories, setSelectedCategories, allCategories }) => {

  const handleChange = e => {
    const checkboxValue = e.target.value;
		if (selectedCategories.includes(checkboxValue)) {
			// Remove checkbox value if already selected
			setSelectedCategories(selectedCategories.filter(item => item !== checkboxValue));
		} else {
 ...
 const closeModal = () => {
  localStorage.setItem('categories', categories[0].category)
  setIsCategoriesModalOpen(false);
  }
     ...
     <div className='fixed top-0 left-0 w-screen h-screen z-10 bg-primary flex justify-center align-middle'>
      ...
      <div className='p-12 py-40 md:p-40 bg-primary my-auto relative rounded-lg w-screen h-[75vh]'>
        <div className='flex flex-col gap-4 md:justify-center md:align-middle md:p-20 xl-p-40'> 
           <h4 className='text-dark font-semibold md:text-lg lg:text-2xl'>Select landmark categories: </h4>
